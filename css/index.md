...
1. scoped  
最常见的模式，通过在style标签上添加scoped属性，让样式只作用于当前组件。Vue 会为该组件的所有元素添加一个唯一的自定义属性，通常是类似data-v-hash值的形式。
2. BEM  
全称Block-Element-Modifier，是由Yandex团队提出的前端CSS命名方法论，旨在通过规范命名规则提升代码可维护性与团队协作效率
3. CSS Modules  
允许你将CSS类名编译成唯一的标识符（每个 class 会被哈希化），从而避免全局命名冲突。
4. Shadow DOM  
得益于DOM的高度隔离性,它非常适合在做组件库或者微前端架构的时候使用,这样可以保证你的样式隔离,不会被奇奇怪怪的全局样式污染.
...
13. [渐变色边框](https://blog.csdn.net/weixin_45092437/article/details/128210309)
